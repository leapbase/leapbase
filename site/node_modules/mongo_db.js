...
var Database = function(app, callback) {
  this.app = app;
  ...
  this.uri = app.setting.database.uri;
  // construct uri if not present
  if (!this.uri) {
    if (this.username && this.password) {
      this.uri = `mongodb://${this.username}:${this.password}@${this.host}`;    
    } else {
      this.uri = `mongodb://${this.host}`;
    }
    if (this.port) {
      this.uri = `${this.uri}:${this.port}`
    }
  }
  debug('db uri:', this.uri);
  // connect to mongo database
  mongoDb.MongoClient.connect(this.uri, function(err, client) {
...
Database.prototype.add = function(table, docs, callback) {
  ...
  this.openTable(table, function(error, tableObject, info) {
    if (error) {
      ...
      // for _id field, need to convert to ObjectID type
      for (var i in docs) {
        var doc = docs[i];
        doc._class = table;
        doc._id = ObjectID();
...
Database.prototype.find = function(table, condition, filter, callback) {
  ...
  try {
    this.openTable(table, function(error, tableObject, info) {
      ...
      tableObject.find(condition, {}, function(error, cursor) {
        cursor.count(function(error, count) {
          ...
          tableObject.find(condition, filter, function(error, cursor) {
            ...
            cursor.toArray(function(error, docs) {
              info = { total:total, module:table, filter:filter };
    ...
    debug('Error in database find:', error);
    callback(error, [], { message:'error in database query' });
  }
};
