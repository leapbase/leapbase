'use strict';
var debug = require('debug')('web_engine');
...
module.exports = function(app) {
    ...
    if (files && files.length > 0) {
      for (var i = 0; i < files.length; i++) {
        ...
        result += '<input type="checkbox" checked="checked"' +
          ...
          'value="' + files[i].path + '" /> ' +
          util.format(template, fileUrl, files[i].originalname) + '</br>';
      }
    }
    return result;
  }
  ...
  function getFileViewTag(property, value, model, cssclass) {
    ...
    var result = util.format(template, cssclass, valueString);
    return result;
  }
  ...
  function getArrayFileViewTag(property, files, model, cssclass) {
    var results = [];
    for (var i = 0; i < files.length; i++) {
      var result = getFileViewTag(property, files[i], model, cssclass);
  ...
  function getObjectViewTag(property, value, model, cssclass) {
    var result = '';
    var template = '';
    var subtype = model.subtype && model.subtype.type || 'json';
    if (value) {
      if (subtype == 'json') {
        value = JSON.stringify(value, null, 4);
      }
  ...
  engine.castData = function(input, model) {
    ...
    switch(model.type) {
      ...
      case 'array':
        ...
        break;
      case 'file':
        result = castFileData(input, model);
        break;
      case 'object':
  ...
  /*
  [ { fieldname: 'photo',
      originalname: 'IMG_9697.JPG',
      encoding: '7bit',
      mimetype: 'image/jpeg',
      destination: 'site/public/upload/',
      filename: 'module-photo-1510726527904',
      path: 'site/public/upload/module-photo-1510726527904',
      size: 1640651 } ]
  */
  function castFileData(input, model) {
    var results = [];
    for (var i = 0; i < input.length; i++) {
      var file = input[i];
      file.url = file.path.replace('site/public', '');
  ...
  function castArrayData(input, model) {
    ...
    // if input is a string, try to parse into array
    if (input.constructor.name == 'String') {
      try {
        input = JSON.parse(input);
      } catch(e) {
        debug('castArrayData - error in parsing input:', input);
      }
    }
    if (input.constructor.name == 'Array') {
      if (subtype && subtype ==='file') {
        result = castFileData(input);
      } else {
        result = input;
      }
    } else {
  ...
  function castObjectData(input, model) {
    ...
    if (input.constructor.name == 'String') {
      try {
        ...
        debug('castObjectData - error in parsing input:', input);
      }
    }
    switch(subtype) {
      case 'json':
